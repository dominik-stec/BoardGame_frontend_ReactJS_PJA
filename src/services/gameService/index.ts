...
class GameService {
  public async joinGameRoom(socket: Socket, playerName: string, playersCount: number): Promise<boolean> {
    return new Promise((rs, rj) => {
      socket.emit("join_game", { 'playerName': playerName, 'playersCount': playersCount });
  ...
  public async onUserLeave(socket: Socket, listiner: (message: string) => void) {
    ...
    // socket.once("user_leave", ({message }) => listiner(message));
  }
